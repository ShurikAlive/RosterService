...
type RosterServer struct {
	...
	eventManager *RosterEventListener
}
...
func CreateRosterServer(connection *DB.Connection, unitService *Swagger.APIClient, equipmentService *Swagger.APIClient, channelRebbitMQ *amqp.Channel, queueRebbitMQ amqp.Queue) (*RosterServer, error) {
	...
	rosterUnitOfWorkFactory := MySQL.NewRosterUnitOfWorkFactory(connection)
	rosterServer.app = App.CreateRosterApp(db, rosterUnitService, rosterEquipmentService, rosterUnitOfWorkFactory)
	rosterServer.eventManager = CreateRosterEventListener(channelRebbitMQ, queueRebbitMQ, &rosterServer.app)
	rosterServer.eventManager.StartListen()
